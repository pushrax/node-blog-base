...
Blog.prototype.getPostByDateName = function(year, month, day, name) {
	console.log(this.postNames[year][month][day]);
	return this.posts[this.postNames[year][month][day][name]];
};

Blog.prototype.getPostsByTag = function(tag) {
	var taggedPosts = [];
	if (tag in this.postTags) {
		for (var i = 0, l = this.postTags[tag].length; i < l; ++i) {
...
Blog.prototype.reloadPosts = function() {
	...
	files.forEach(function (file) {
		...
		fs.readFile(path.join(self.dir, file), 'utf-8', function(err, str) {
			...
			var date = new Date(parseInt(fileName.substr(0, 4)), parseInt(file.substr(5, 2)), parseInt(file.substr(8, 2)));
			...
			self.tempPosts.push({ name: fileName.substr(11), title: data.meta.title, tags: tags, date: date, synopsis: synopsis, html: html });
